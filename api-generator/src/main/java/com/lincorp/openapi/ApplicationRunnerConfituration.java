...
@Configuration
public class ApplicationRunnerConfituration {
    @Autowired
    private Shell shell;

    @Bean
    public CommandLineRunner exampleCommandLineRunner(ConfigurableEnvironment environment) {
        return new GeneratorCommandLineRunner(shell, environment);
    }
}

@Order(InteractiveShellApplicationRunner.PRECEDENCE)
class GeneratorCommandLineRunner implements CommandLineRunner {

    private Shell shell;

    private final ConfigurableEnvironment environment;

    public GeneratorCommandLineRunner(Shell shell, ConfigurableEnvironment environment) {
        this.shell = shell;
